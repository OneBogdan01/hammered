...
hm::alloy::CommandBuffer& hm::alloy::CommandBuffer::add_circle(const Circle circle,
                                                                   const uColor32 color) {
    const UICommand cmd{circle, color};
    m_commands.push_back(cmd);
    return *this;
}
hm::alloy::CommandBuffer& hm::alloy::CommandBuffer::add_rect(const Rect rect,
                                                                 const uColor32 color) {
    const UICommand cmd{rect, color};
    m_commands.push_back(cmd);
    return *this;
}
hm::alloy::CommandBuffer& hm::alloy::CommandBuffer::add_line(const Line line,
                                                                 const uColor32 color) {
    const UICommand cmd{line, color};
    m_commands.push_back(cmd);
    return *this;
}
hm::Vec<hm::alloy::UICommand>& hm::alloy::CommandBuffer::get_commands() {
    return m_commands;
}